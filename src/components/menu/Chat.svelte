<script lang="ts">
	// Chat (phase 67): message bubbles with author color chips + timestamps,
	// own messages right-aligned, Enter to send, autoscroll with a new-messages
	// pill, and inline /command hints. Send path unchanged ($peers.sendMessage).
...
<div id="chat" class={$chatHidden}>
	<div id="chat-window" class="flex flex-col rounded-lg border border-gray-700/60 bg-gray-900/80 shadow-xl backdrop-blur">
		<div id="chat-handle" class="drag-handle"></div>

		<div id="chat-messages" bind:this={scroller} onscroll={onScroll} class="flex-1 overflow-y-auto px-2 py-1">
	...
	{#if !$isLocked}
		<div id="chat-input" class="rounded-lg border border-gray-700/60 bg-gray-900/85 shadow-xl backdrop-blur">
			{#if hints.length}
