...
- **`openedPeers` is a SET, so `.length` is a silent always-false.** It was wrong in three
  places at once, including a connect prompt that could therefore never appear.
  `Connect.svelte` spreads it (`[...$peers.openedPeers]`), which is why nothing else had
  hit it. The check written to prove WHY a fix worked is what caught it — assert the REASON,
  not only the effect.
- **`showInfoToast` used to return EARLY when its id was already up**, so a sticky card
  could never change its own text or buttons. Every one of those cards is a MIRROR OF
  STATE, so that made an inline confirm impossible: the first press armed the flag, the
  effect re-ran, and the label stayed put — reported as "the button does nothing". It
  updates in place now, and toast actions take `keepOpen` (without it every press dismisses
  the card, so a button that arms a second press closes what it is arming).
- **A DELETE IS AN EVENT TO APPLY ONCE, NOT A STANDING INSTRUCTION.** The deleted-log
  applier ran on every library sweep over the WHOLE log, and the log persists in idb — so
  any file whose hash had ever been deleted was re-hidden the instant anything touched the
  library, including re-importing it and pressing Share. Record what you have applied and
  PERSIST that record, or a reload replays every deletion the project ever had.
- **A pill whose LABEL appears and disappears on a timer is never "stable" to click** —
  playwright refused the press for a full 30s. Carry a transient state with colour and an
  icon, never with text that resizes the control.
- **Naming a local `state` makes every `$state` rune parse as a STORE reference** (the `# CLAUDE.md — theprototype.app
