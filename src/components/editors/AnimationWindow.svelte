<script>
	// Animation window v1 (roadmap #9 tail): a LOCAL-ONLY transform animator for the
	// selected object. Left = movement tracks (layers), centre = a cubic-bezier
	// easing-curve editor (draggable dots = the speed graph, like After Effects),
	// right = the selected track's properties. Transport previews it by driving the
	// object per frame (animationPreview.js). Floating window that tab-groups with
	// Flow (tabbable) + tears off (detach), mirroring FlowCode.
	...
	// (restores its base pose). Depends only on `target`; the playback read + store
	// writes are untracked so this can't self-retrigger.
	...
	/** value shown in the from/to fields (rotation in degrees) */
	...
	let dragIdx = $state(-1); // 0 = P1 (x1,y1), 1 = P2 (x2,y2)
	function onHandleDown(/** @type {number} */ idx, /** @type {PointerEvent} */ e) {
		dragIdx = idx;
		e.preventDefault();
		// pointermove/up on window: a mid-drag pointerup can land outside the small svg
...
{#if !$animationClose}
	<div
		...
		style="z-index: var(--z-window); width: 660px; height: 460px; max-width: 96vw; max-height: 88vh"
	>
