...
h.run(async () => {
	...
	// content everywhere: objects from both sides + pong module content
	...
	await h.eventually(() => pongPresent(B.page), (v) => v === true, 'pong content on B');
	...
	await h.eventually(() => pongPresent(A.page), (v) => v === false, 'module content cleared on A');
	await h.eventually(() => pongPresent(B.page), (v) => v === false, 'module content cleared on B');
